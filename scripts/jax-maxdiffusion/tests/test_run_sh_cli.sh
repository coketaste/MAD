#!/usr/bin/env bash
# Regression: MAD jax-maxdiffusion launches Primus via primus-cli, not examples/run_pretrain.sh.
set -euo pipefail
...
assert_file_contains "$RUN_SH" 'MAXDIFFUSION_PATH'

DOCKERFILE="$SCRIPT_DIR/../../../docker/primus_maxdiffusion.ubuntu.amd.Dockerfile"
assert_file_contains "$DOCKERFILE" '/workspace/Primus/primus-cli'
assert_file_not_contains "$DOCKERFILE" 'examples/run_pretrain.sh'

echo "jax-maxdiffusion run.sh uses primus-cli."
